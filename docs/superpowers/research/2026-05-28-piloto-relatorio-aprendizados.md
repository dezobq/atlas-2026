...
3. **[PRÉ-CONDIÇÃO] `pnpm archive` (Save Page Now) falhando** para todos os domínios testados (status 200 sem URL de snapshot) — provável mudança/auth na API do Wayback. No piloto, usei snapshots **já existentes** via API de disponibilidade (`archive.org/wayback/available`). **Investigar/consertar `scripts/archive.ts` antes do lote** (60 declarações dependem de archive confiável).
